    ...
    while True:
        try:
            async with websockets.connect(ALCHEMY_WS_URL, ping_interval=20, ping_timeout=20) as ws:
                ...
                # Subscribe to model registry logs (address-filtered only for now).
                await ws.send(
                    json.dumps(
                        {
                            "jsonrpc": "2.0",
                ...
                # Subscribe to job manager logs (address-filtered only for now).
                await ws.send(
                    json.dumps(
                        {
                            "jsonrpc": "2.0",
...
def list_jobs(current_user: dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        # Fallback when relation filters are unavailable: map wallet -> node ids -> jobs.
        ...
        try:
            ...
            # Alternate wallet column naming.
            node_rows = (
                supabase.table("nodes")
                .select("id")
